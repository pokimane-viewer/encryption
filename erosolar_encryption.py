...
import pgpy, os, uuid, secrets, hashlib, base64, time, tempfile
import gnupg
            ...
            if action == 'encrypt':
                ...
                if signed_message_blob and MY_PUBLIC_PGP_KEY_OBJ:
                    try:
                        ...
                        import_result = gpg.import_keys(MY_PGP_PUBLIC_KEY)
                        if not import_result.count:
                             raise ValueError("Failed to import my public key into temporary GPG keyring.")

                        verified = gpg.verify(signed_message_blob)
                        if verified and verified.valid:
                             verification_result = True
                             verified_message_content = gpg.decrypt(signed_message_blob).data.decode('utf-8', errors='replace') if verified.data else "[Could not extract verified message content]"
                        else:
                             verification_result = False
                             verify_error = verified.status if verified else "Verification failed (unknown reason)"
                             if verified and not verified.valid:
                                 verify_error = f"Signature INVALID. Status: {verified.status}"

                    except ValueError as e:
                        verify_error = f"Invalid PGP message format or signature: {str(e)}"
                        verification_result = False
                    except Exception as e:
                        verify_error = f"PGP verification failed with GnuPG: {str(e)} ({type(e).__name__})"
                        verification_result = False
                elif not MY_PUBLIC_PGP_KEY_OBJ:
